...
0x18 & \texttt{ecc\_check} & Read/write: low byte is the Hamming check byte\\
...
The following listing reproduces \file{kernel/src/shbt_core_runtime.c}, the normative freestanding C11 microkernel.  It depends on the register layout defined in \file{kernel/include/shbt_hardware.h} (Section~\ref{sec:mmio}).
...
The AVX-512 Givens rotation remap is implemented in \file{kernel/src/shbt_core_runtime.c} as \texttt{shbt\_remap(double *col\_a, double *col\_b, double c, double s, size\_t n)}.  Each 512-bit vector processes eight \texttt{double} lanes; the scalar tail handles the remaining $n \bmod 8$ elements.  The transformation is
\[
 a'_i = c\,a_i + s\,b_i,\qquad b'_i = c\,b_i - s\,a_i.
\]
This preserves the column norm to binary64 unit roundoff and corrects the earlier row-major float formulation and missing tail mask.  See Listing~\ref{lst:core} for the source.
...
V-05 & Boundary isometry operator norm & Operator state tomography & $\|W^\dagger W-P_\code\|_\op$ including uncertainty; verified by \file{formal/formal_verification.py} (Z3 \texttt{verify\_isometry\_bound}) over 312 channels & $\le3.430\times10^{-3}$ (revised ceiling) & Nonunitary channel recalibration or explicit recovery projection; unitary remap alone cannot improve defect\\
...
V-27 & Post-quench recovery time & Nanosecond time-interval counter and binary WCET audit & Four-step recovery in \file{kernel/src/shbt_core_runtime.c}; stage maxima $(40,65,115,80)$ cycles; exercised by \file{tests/run_all_tests.py} and \file{formal/formal_verification.py} (Z3 \texttt{verify\_liveness\_invariant}) & $t_{\mathrm{rec}}\le120.00\unit{ns}$ at $f\ge2.5\unit{GHz}$; no omitted delays & Reduce and measure complete-stage maxima; unrecoverable faults fail closed, not pass\\
...
V-34 & SECDED ECC decode execution time & C microkernel \file{kernel/src/shbt_core_runtime.c} / VPU cycle-accurate simulator / ELA & Correction-cycle duration; single-bit correction and double-bit detection exercised in \file{tests/reference_test.c} and \file{tests/run_all_tests.py} & $t_{\mathrm{ecc}}\le1.20\unit{ns}$ & Synthesize dual-rail logic\\
...
The code artifacts discharge the following V\&V targets in this revision:
\begin{itemize}[noitemsep]
\item V-05: \file{formal/formal_verification.py} proves the 312-channel isometry operator-norm defect remains below the revised ceiling $3.430\times10^{-3}$ (fit value $3.42640\times10^{-3}$).
\item V-27: \file{kernel/src/shbt_core_runtime.c} implements the four-step recovery sequence and \file{formal/formal_verification.py} proves the stage cycle envelope $(40,65,115,80)$ at $f\ge2.5$~GHz meets $t_{\mathrm{rec}}\le120.00$~ns.
\item V-34: \file{kernel/src/shbt_core_runtime.c} \texttt{shbt\_ecc\_decode} executes SECDED Hamming(72,64) correction/detection within the $1.20$~ns target budget on the reference VPU.
\end{itemize}
...
\subsection{Code repository and build verification}
The C reference implementation is maintained in the repository as \file{kernel/src/shbt_core_runtime.c}, with the user-space MMIO relocation shim in \file{kernel/src/shbt_user_mmio.c} and the normative register header in \file{kernel/include/shbt_hardware.h}.  The CLI build command compiles the microkernel into a shared object:
\begin{lstlisting}[language=bash]
python3 cli/main.py build-kernel
\end{lstlisting}
Equivalent manual compilation is:
\begin{lstlisting}[language=bash]
gcc -O3 -std=c11 -ffreestanding -nostdlib -fPIC -shared -mavx512f \
  -DSHBT_TSC_HZ=2400000000ULL -I kernel/include \
  kernel/src/shbt_user_mmio.c -o shbt_reference.so
readelf -h shbt_reference.so
\end{lstlisting}

\subsection{Hosted functional test driver}
The hosted C test suite now lives in \file{tests/reference_test.c}.  It pulls in the user-space MMIO relocation shim from \file{kernel/src/shbt_user_mmio.c}, exercises single-bit correction and double-bit detection for SECDED Hamming(72,64), the post-quench recovery sequence, and AVX-512 Givens remap verification, and is executed by \file{tests/run_all_tests.py} and the \texttt{shbt-tool verify} command.  The CLI is invoked with \texttt{python3} \file{cli/main.py} \texttt{build-kernel}, \texttt{python3} \file{cli/main.py} \texttt{verify}, and \texttt{python3} \file{tests/run_all_tests.py}.

\paragraph{Checks executed for this revision.}
The repository build and test chain was run successfully: \file{cli/main.py} \texttt{build-kernel} produced \file{shbt_reference.so} with a valid ELF64 shared-object header, \texttt{verify} ran the Z3 formal audit and the pytest closed-loop bench, and \file{tests/run_all_tests.py} passed all seven suites (C reference, multi-physics FEA, photonic CAD, HIL/MPS-QEC, pytest bench, Z3 formal verification, and EDA exporter integrity).  PDFLaTeX compilation of this manuscript succeeded.  These checks validate the implemented software artifacts and conditional contracts; they do not by themselves close hardware timing, fabrication, or quantum-performance claims.
